...
async function resolveAbiFromBytecode(contract) {
  const [{ resolveImplementation }, { extractIPFSUri }] = await Promise.all([
    import("./resolveImplementation-6VVWY26C.js"),
    import("./extractIPFS-CYG35U2N.js")
  ]);
...
async function resolvePluginPatternAddresses(contract) {
  try {
    const { readContract } = await import("./read-contract-HOL5QHH7.js");
...
async function resolveModularModuleAddresses(contract) {
  try {
    const { getInstalledModules } = await import("./getInstalledModules-NUW7C2DA.js");
...
async function estimateGas(options) {
  ...
  const promise = (async () => {
    ...
    const { encode: encode2 } = await import("./encode-TNWPXX4Z.js");
    ...
    const [{ getRpcClient: getRpcClient2 }, { eth_estimateGas }] = await Promise.all([
      import("./rpc-QV33PVVK.js"),
      import("./eth_estimateGas-5EBM2WH5.js")
    ]);
...
async function toSerializableTransaction(options) {
  ...
  let [data, nonce, gas, feeData, to, accessList, value] = await Promise.all([
    ...
    (async () => {
      ...
      return from ? await import("./eth_getTransactionCount-6BCNMIDG.js").then(({ eth_getTransactionCount }) => eth_getTransactionCount(rpcRequest, {
        address: from,
        blockTag: "pending"
      })) : void 0;
    })(),
...
//# sourceMappingURL=chunk-JOTM2E7T.js.map
